...
struct PackageManagerView: View {
    @State private var model = PackageManagerViewModel()
    var onBack: () -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text("📦 Package manager").font(.title2.bold())
                Spacer()
            }
            ...
            switch model.state {
            case .idle, .loading:
                VStack { Spacer(); ProgressView("Ricerca cache e node_modules…"); Spacer() }
                    .frame(maxWidth: .infinity)
            case .loaded(let findings):
                if findings.isEmpty {
                    ContentUnavailableView("Niente da recuperare", systemImage: "checkmark.circle",
                                           description: Text("Nessuna cache o node_modules trovata."))
                } else {
